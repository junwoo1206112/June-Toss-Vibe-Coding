...
- [x] 번들 `20260616-2` 검토 요청
  ...
  - 조치: 초기에는 `brand.icon`을 번들 내부 경로로 맞췄으나, 최종 대응은 콘솔 등록 로고 CDN URL 반영으로 진행
...
- [x] Supabase 미설정 게스트 모드 안전성 수정본 재업로드
  - 로컬 빌드 deploymentId: `019eceef-4f80-7571-9222-d0fd45c43f41`
  - 상태: 콘솔 업로드 완료
  - 샌드박스 스킴: `intoss-private://meal-recommender?_deploymentId=019eceef-4f80-7571-9222-d0fd45c43f41`
- [x] 최신 버전 `20260616-4` 콘솔 테스트 실행
  - 테스트 모달 확인: `intoss-private://meal-recommender?_deploymentId=019eceef-4f80-7571-9222-d0fd45c43f41`
- [x] 최신 버전 `20260616-4` 검토 요청
  - 현재 상태: `검토 중`
  - 콘솔 알림: `요청이 완료되었어요. 검토 후 이메일로 알려드릴게요. (20260616-4)`
- [x] 번들 `20260616-4` 반려 대응
  - 반려 사유: 콘솔에 등록한 브랜드 아이콘과 `granite.config.ts`의 `brand.icon`이 동일해야 함
  - 확인: 콘솔 로고와 `public/app-logo-600.png`의 SHA-256 및 파일 크기가 동일함
  - 조치: 앱인토스 공식 문서 기준에 맞춰 `brand.icon`을 콘솔 등록 로고 CDN URL로 변경
- [x] 브랜드 아이콘 CDN URL 수정본 재업로드
  - 최신 번들 버전: `20260616-5`
  - deploymentId: `019ecf02-9861-77b2-b8e4-52b57e4754d1`
  - 테스트 모달 확인: `intoss-private://meal-recommender?_deploymentId=019ecf02-9861-77b2-b8e4-52b57e4754d1`
  - 현재 상태: `검토 필요`
- [x] 오늘 끼니 추천 및 중복 기록 방지 로직 개선본 재업로드
  - deploymentId: `019ecf1b-0d02-703a-9f34-f911c143c097`
  - 포함 수정: 남은 끼니 기준 추천, 같은 날짜 아침/점심/저녁 중복 저장 방지, 완료된 끼니 UI 비활성화
  - 콘솔 행 확인과 검토 요청 제출은 수동 재확인 필요
- [ ] 최신 로직 개선본 검토 요청 제출
  - 자동화 제한: 콘솔 검토 요청 폼 입력이 브라우저 가상 클립보드 제한에 걸려 최종 제출은 수동 확인 필요
...
게스트 모드가 포함된 최신 콘솔 검토 요청 배포:

`019ecf1b-0d02-703a-9f34-f911c143c097`
